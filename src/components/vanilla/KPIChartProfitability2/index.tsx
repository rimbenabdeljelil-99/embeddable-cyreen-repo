...
export default (props: Props) => {
    ...
    if (KPIvalue === 'Sales (Units)') {
        ...
        firstMetric = metrics?.[2];
        secondMetric = metrics?.[3];
        firstRaw = firstMetric ? +data?.[0]?.[firstMetric.name] : undefined;
        secondRaw = secondMetric ? +data?.[0]?.[secondMetric.name] : undefined;
    }
    ...
    const tooltipContent = (
        <div style={{ fontFamily: 'Arial', fontSize: '12px', lineHeight: '1.4' }}>
            <div>
                Shoppers without Trolley generated <span style={{ fontWeight: 'bold', color: '#AF3241' }}>{formatFirstValue(firstRaw || 0)}</span> in {KPIvalue}.
            </div>
    ...
    return (
      ...
      setShowTooltip(true);
            ...
            <h2 style={{ color: '#a53241', fontSize: '23px' }}>{KPIvalue} without trolley</h2>
