...
>(Component, meta, {
  props: (inputs: Inputs<typeof meta>, [state]) => {
   ...
   const visibleDimensions = (inputs.columns?.filter(isDimension) as Dimension[] || []).filter((col, index) => {
  //if (index === 0 && inputs.FirstColumn) return false;
  //if (index === 1 && inputs.SecondColumn) return false;
  if (index === 2 && inputs.ThirdColumn) return false;
  if (index === 3 && inputs.FourthColumn) return false;
  if (index === 4 && inputs.FifthColumn) return false;
  return true;
});

const visibleMeasures = (inputs.columns?.filter(isMeasure) as Measure[] || []).filter(col => true);

const filteredSort = (state?.sort || defaultSort).filter(
  (s) => visibleDimensions.includes(s.property as Dimension) || visibleMeasures.includes(s.property as Measure)
);
