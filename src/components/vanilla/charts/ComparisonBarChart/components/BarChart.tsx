...
export default function ComparisonBarChart({ ...props }: Props) {
    const primaryChartRef = useRef<ChartJS>(null);
    const comparisonChartRef = useRef<ChartJS>(null);
    
    const primaryChartData = useMemo(() => chartData({ ...props, results: props.primaryResults }), [props, props.primaryResults]);
    const comparisonChartData = useMemo(() => chartData({ ...props, results: props.comparisonResults }), [props, props.comparisonResults]);
    ...
    const toggleDataset = (datasetIndex: number) => {
        ...
        [primaryChartRef.current, comparisonChartRef.current].forEach(chart => {
            if (chart) {
    ...
    const needsScrollableContainer = primaryChartData.labels.length > 200 || comparisonChartData.labels.length > 200;
    ...
    if (props.primaryResults?.isLoading || props.comparisonResults?.isLoading) {
        return <div style={{ height: '100%' }} />;
    }

    // Check if xAxis exists in data for primary results
    ...
    if (props.primaryResults?.data?.length > 0 && props.xAxis) {
        const firstDataItem = props.primaryResults.data[0];
    ...
    // Prepare legend items (using primary chart data since both charts have same datasets)
    const createLegendItems = (chartDataResult: ChartData<'bar' | 'line'>) => {
        ...
        const items = chartDataResult.datasets.map((ds, originalIndex) => ({
            label: labelMap[ds.label] || ds.label,
            color: ds.backgroundColor,
            originalIndex,
            displayIndex: shouldReverse
                ? chartDataResult.datasets.length - 1 - originalIndex
                : originalIndex
        }));
    ...
    const legendItems = createLegendItems(primaryChartData);

    const renderChart = (chartRef: React.RefObject<ChartJS>, chartDataResult: ChartData<'bar' | 'line'>, groupLabel: string, showIcon?: boolean, iconPosition?: 'left' | 'right', additionalProps?: any) => {
    ...
    // Disable the default Chart.js legend
    const optionsWithNoLegend = {
        ...chartOptions,
        plugins: {
            ...chartOptions.plugins,
            legend: {
                display: false // This disables the default Chart.js legend
            }
        }
    };
        ...
        <div style={{ position: 'relative', height: '100%' }}>
            ...
            {showIcon && (
                <div style={{
                    ...
                    <img 
                        src={iconPosition === 'left' ? BIcon : AIcon} 
                        alt={iconPosition === 'left' ? 'B Icon' : 'A Icon'}
    ...
    const renderScrollableChart = (chartRef: React.RefObject<ChartJS>, chartDataResult: ChartData<'bar' | 'line'>, groupLabel: string, showIcon?: boolean, iconPosition?: 'left' | 'right') => {
        ...
        return (
            ...
            >
                {/* Group Label for Scrollable Chart - Simple Text */}
                <div style={{
                    position: 'absolute',
                    top: '10px',
                    left: '50%',
                ...
                >
                    <div style={{ display: "flex", position: "relative", width: "100%" }}>
                        ...
                        <div style={{ flexGrow: 1 }}>
                            ...
                            >
                                {renderChart(chartRef, chartDataResult, groupLabel, showIcon, iconPosition)}
                            </div>
                        </div>
    ...
    // Main container for side-by-side charts with single legend
    return (
        <div style={{
            display: 'flex',
            flexDirection: 'column',
            ...
            <div style={{
                ...
                flex: 1,
                gap: '20px',
                marginBottom: props.showLegend ? '0' : '20px'
            }}>
                {/* Primary Chart */}
                <div style={{
                    flex: 1,
                    display: 'flex',
                    flexDirection: 'column',
                    borderRight: '2px solid #af3241',
                    paddingRight: '20px'
                }}>
                    <div style={{ flex: 1, minHeight: '400px', position: 'relative' }}>
                        {needsScrollableContainer ? 
                            renderScrollableChart(primaryChartRef, primaryChartData, "Group A", true, 'right') : 
                            renderChart(primaryChartRef, primaryChartData, "Group A", true, 'right')
                        }
                    </div>
                </div>
                ...
                <div style={{
                    ...
                    <div style={{ flex: 1, minHeight: '400px', position: 'relative' }}>
                        {needsScrollableContainer ? 
                            renderScrollableChart(comparisonChartRef, comparisonChartData, "Group B", true, 'left') : 
                            renderChart(comparisonChartRef, comparisonChartData, "Group B", true, 'left')
                        }
                    </div>
                </div>
            </div>

            {/* Single Centered Legend */}
            {props.showLegend && renderLegend()}
        </div>
    );
}

// Keep the existing chartData function unchanged
function chartData(props: Props & { results?: DataResponse }): ChartData<'bar' | 'line'> {
    // ... (keep the exact same chartData function as before)
    // [The complete chartData function remains unchanged]
