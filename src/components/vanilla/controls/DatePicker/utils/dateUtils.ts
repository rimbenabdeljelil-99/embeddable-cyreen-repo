...
export function getComparisonOptions(period: TimeRange) {
  ...
  return [
    ...
    {
      value: 'Custom period', // ✅ new option for manual selection
      note: 'Select custom date range',
    },
  ];
}
...
export function getComparisonPeriod(rts: string, period: TimeRange) {
  ...
  } else if (rts === 'Custom period') {
    // ✅ User will select manually — no auto-calculation
    return {
      relativeTimeString: 'custom period',
      from: new Date(),
      to: new Date(),
    };
  } else {
